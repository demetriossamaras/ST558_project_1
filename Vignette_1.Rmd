...
#### This function will use our `WhosThatPokemon` function to pull whichever pokemon are desired. For example say you want 25 - 50, or a random sample of 30 pokemon and their heights. This function takes a vector of pokemon names or ID's. User can supply either a vector of ID's or a vector of names 
...
GiveMeThesePokemon <- function(pokemons, info=NULL){
  ...
  index <- index[pokemons, ]
  
  
  ThesePokemon <-sapply(X=pokemons, FUN = WhosThatPokemon, info = info)
  ThesePokemon <- unlist(ThesePokemon)
  
  assign(eval(info), "value")
  
  p<- data.frame(name=index$name, ID=index$ID , value=ThesePokemon)
