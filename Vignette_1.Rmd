...
This function will use our 6 endpoint functions to pull info from whichever endpoint is desired by setting fun=desired function to help with our EDA. For example say you want costs for items ID 25 - 42 or a random sample of pokemon and their heights. This function takes a vector of names **or** a vector of ID's that are supplied by the user and info = "desired info". The desired info must be an integer, for example height, weight, size, cost, etc. 
...
Here we will look at the size distribution of all berries 
...
Here we will categorize the berries into size and look at how many fall into each group. We can see that the largest group of berries fall into the small size category, none are tiny and a similar amount fall into medium and large. 
...
## types returns more that one result so we will use sapply to get types 
...
Here we can look at the relationship between the heights and the weights of Pokemon based on there types. We can see that in general as weights increase so do heights but there are some extreme examples of both that dont follow the trend line. The doesnt seem to be an obvious relationship between type and either size or weight.
...
s+geom_point(aes(color=type)) + geom_smooth(method = lm) +labs(y= "Height (dm)", x="Weight(hg)")+ggtitle("Height and Weight by type")
